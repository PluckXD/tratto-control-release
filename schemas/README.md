...
the exact production-policy digest, immutable controller tag/release
bindings, and linear continuity with the separately protected approval
ledger. `scripts/validate-ledger.py` replays every historical approval and
validates repository/ref/genesis/parent/sequence continuity before emitting a
canonical summary.
...
revalidation. `scripts/validate-signer-freshness-v2.py` requires typed,
authenticated initial and fresh observations; embedded JSON is never accepted
as its own proof.
